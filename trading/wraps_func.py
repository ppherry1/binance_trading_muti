...
def try_catch(func):
    ...
    def wrapfunc(self, *args, **kwargs):
        try:
            ...
            print(e)
            raise('命令%s执行错误,raise Exception,参数:' % func.__name__, args, kwargs)
            raise Exception
            ...
            try:
                if self.exchange_name == 'BINANCE':
                    ...
                    if func_name in ['get_spot_account']:
                        ...
                        res = res['assets']
                    elif func_name in ['get_exchangeInfo']:
                        res = res['symbols']
                    elif func_name in ['get_future_positions']:
                        res = res['positions']
                        ...
                        for item in res:
                            if 'USDT' not in item['symbol']:
                                ...
                                item['usd_profit'] = float(item['positionAmt']) * face_value * (
                                            float(item['markPrice']) / float(item['entryPrice']) - 1) if float(item['entryPrice']) != 0 else 0
...
def webbrowser_show(func):
    ...
    def wrapfunc(self, *args, **kwargs):
        # try:
        res = func(self, *args, **kwargs)
        # res.to_html('./data/tmp_shower.html', encoding='gb18030')
        # print(root_path)
        # print(root_path + '/../data/tmp_shower.html')
        # webbrowser.open(root_path + '/../data/tmp_shower.html')
        return res
        # except Exception as e:
        #     print(e)
        #     print('结果转换格式出错，raise Exception')
